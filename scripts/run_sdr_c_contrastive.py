...
logging.basicConfig(level=logging.INFO, format="%(asctime)s [sdr-c] %(levelname)s %(message)s",
                    datefmt="%H:%M:%S")
...
def main():
    ...
    log.info("baselines  dense: resnik=%.4f lin=%.4f | raw-binary: resnik=%.4f lin=%.4f",
             base_rho[("resnik", "dense_cosine")], base_rho[("lin", "dense_cosine")],
             base_rho[("resnik", "raw_magnitude_binary")], base_rho[("lin", "raw_magnitude_binary")])
    ...
    try:
        ...
        mlflow.log_params({"n": n, "dict": args.dict, "k": args.k, "epochs": args.epochs,
                           "lr": args.lr, "objective": "soft-tanimoto-binary", "seed": args.seed})
        _ml = True
    ...
    for ep in range(args.epochs):
        ...
        if ep % 10 == 0 or ep == args.epochs - 1:
            ...
            log.info("  epoch %3d loss=%.4f dens=%.3f | LEARNED-BINARY held-out: resnik=%.4f lin=%.4f "
                     "(dense %.4f / raw-bin %.4f)", ep, tot, dens, er, el,
                     base_rho[("resnik", "dense_cosine")], base_rho[("resnik", "raw_magnitude_binary")])
    ...
    for metric in ("resnik", "lin"):
        ...
        row = {
            ...
            "learned_binary_topk": float(
                spearmanr(tanimoto_dense(topk_binary_general(Zev, args.k), args.k)[ii, jj], g)[0]),
        }
        log.info("--- FINAL %s (held-out n=%d, %d pairs) ---", metric, len(ev), len(epairs))
        for name, rho in row.items():
            log.info("  %-22s rho=%.4f", name, rho)
        log.info("  >> LEARNED binary: %+.4f vs dense, %+.4f vs raw-binary",
