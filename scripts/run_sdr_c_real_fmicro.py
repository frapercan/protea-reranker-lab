...
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s [real-fmicro] %(levelname)s %(message)s",
    datefmt="%H:%M:%S",
)
...
def main():
    ...
    TI = torch.tensor([p[0] for p in tp])
    TJ = torch.tensor([p[1] for p in tp])
    ...
    PATIENCE = 6
    log.info(
        "dense baseline f_micro=%.4f Spearman=%.4f | training to CONVERGENCE (early-stop on f_micro, patience %d)",
        f_dense,
        sp_dense,
        PATIENCE,
    )
    ...
    for e in range(args.epochs):
        ...
        if e % EVAL_EVERY == 0 or e == args.epochs - 1:
            ...
            log.info(
                "  epoch %3d | learned-real(k=%d) f_micro=%.4f vs dense %.4f (%+.4f)",
                e,
                eval_k,
                f_sp,
                f_dense,
            ...
            if f_sp > best_f + 1e-4:
                ...
                if since >= PATIENCE:
                    log.info(
                        "  PLATEAU at epoch %d (best f_micro=%.4f @ %d)",
                        e,
                        best_f,
                        best_ep,
                    )
    ...
    log.info(
        "  dense                      f_micro=%.4f  (Spearman %.4f)", f_dense, sp_dense
    )
    log.info(
        "  learned-real FULL (%d-d)   f_micro=%.4f  (Spearman %.4f)",
        args.dict,
        f_lr_full,
        sp_lr,
    )
    ...
    for k in args.k:
        ...
        log.info(
            "  learned-real SPARSE k=%-4d  f_micro=%.4f  (%+.4f vs dense)",
            k,
            f_sp,
            f_sp - f_dense,
        )
    ...
    terms = sorted({t for c in clo_ev for t in c})
    tix = {t: i for i, t in enumerate(terms)}
    rr, cc = [], []
    for i, cl in enumerate(clo_ev):
        for t in cl:
            ...
            cc.append(tix[t])
    Tev = sp.csr_matrix(
        (np.ones(len(rr), np.float32), (rr, cc)), shape=(len(ev), len(terms))
    )
    Td = np.asarray(Tev.todense()).astype(bool)
    bestk = int(max(args.k, key=lambda k: res[f"lr_sparse_k{k}"]))
    pred_d = transfer_pred(De, Tev)
    pred_l = transfer_pred(topk_real(Zev, bestk), Tev)
    log.info(
        "=== H5 (LEARNED): f_micro by size, dense vs learned-real(k=%d) ===", bestk
    )
    for b, m in {
        "1-chunk": nch_ev == 1,
        "2-chunk": nch_ev == 2,
    ...
    log.info(
        ">> VERDICT: best learned-real = %s f_micro=%.4f vs dense %.4f (%+.4f) | peak-during-train %.4f@%d",
        best,
        res[best],
        f_dense,
        res[best] - f_dense,
