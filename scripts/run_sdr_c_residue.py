...
logging.basicConfig(level=logging.INFO, format="%(asctime)s [sdr-c-res] %(levelname)s %(message)s",
                    datefmt="%H:%M:%S")
...
def main():
    ...
    ap.add_argument("--cap-res", type=int, default=64, help="max residues/protein for the LEARNED arm")
    ...
    log.info("residue-length dist: min=%d med=%d max=%d  (>=2 chunks ~ len>512: %.1f%%)",
             int(lens.min()), int(np.median(lens)), int(lens.max()), 100 * (lens > 512).mean())
    ...
    log.info("naive bundle active-dim mean: real=%.0f binary=%.0f (of %d)",
             float((nb_real > 0).sum(1).mean()), float(nb_bin.sum(1).mean()), d)
    ...
    GO = {"resnik": np.asarray(resnik_pairwise(closures, ic, epairs), dtype=np.float64),
    ...
    for m in ("resnik", "lin"):
        r = res_rows[m]
        log.info("  %-7s dense=%.4f | naive-real(bundle)=%.4f | naive-binary(OR)=%.4f",
                 m, r["dense"], r["naive_real"], r["naive_binary"])
    ...
    def flat_res(indices, cap):
        ...
        for p, gi in enumerate(indices):
            ...
            mats.append(M[sel]); pidx.extend([p] * len(sel))
    ...
    log.info("learned arm units: train=%d residues over %d proteins | eval=%d over %d (cap=%d)",
             Xtr.shape[0], len(tr), Xev.shape[0], len(ev), args.cap_res)
    ...
    ytr = torch.tensor(np.asarray(lin_pairwise(clo_tr, ic_tr, tpairs,
    ...
    iie = np.array([p[0] for p in epairs_e]); jje = np.array([p[1] for p in epairs_e])
    GOe = {"resnik": np.asarray(resnik_pairwise(clo_ev, ic_ev, epairs_e), dtype=np.float64),
           "lin": np.asarray(lin_pairwise(clo_ev, ic_ev, epairs_e, bic_ev), dtype=np.float64)}
    lens_ev = lens[ev]
    ...
    try:
        ...
        mlflow.log_params({"n": n, "dict": args.dict, "k": args.k, "cap_res": args.cap_res,
                           "granularity": "per-residue-bundle", "seed": args.seed})
        for m in ("resnik", "lin"):
    ...
    log.info("training A2 (per-residue learned bundle) on %s: dict=%d k=%d epochs=%d",
             dev, args.dict, args.k, args.epochs)
    bs = 16384; np_ = len(tpairs)
    for ep in range(args.epochs):
        ...
        if ep % 10 == 0 or ep == args.epochs - 1:
            ...
            log.info("  epoch %3d loss=%.4f | A2 held-out resnik=%.4f lin=%.4f "
                     "(dense %.4f / A0 ~0.49 / A1 ~0.48)", ep, tot, er, el, res_rows["resnik"]["dense"])
    ...
    for metric in ("resnik", "lin"):
        ...
        log.info("  %s: A2=%.4f (dense %.4f) | short pairs=%.4f | long(>512) pairs=%.4f",
                 metric, full, res_rows[metric]["dense"], small, large)
        if _ml:
