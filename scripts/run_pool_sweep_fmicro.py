...
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s [pool-sweep] %(levelname)s %(message)s",
    datefmt="%H:%M:%S",
)
...
def p_max(M):
    return M.max(0)  # signed max


def p_min(M):
    return M.min(0)  # signed min
...
def p_absmax(M):
    return np.abs(M).max(0)  # magnitude only, DROP sign


def p_meanabs(M):
    return np.abs(M).mean(0)  # mean of magnitudes
...
def main():
    ...
    base = {
        name: np.vstack([fn(res[a]) for a in acc]).astype(np.float32)
        for name, fn in {
            "mean": p_mean,
            "max": p_max,
            "min": p_min,
    ...
    def cat(
        *names,
    ):  # L2-normalise each component THEN concat, so no single pool dominates the norm
