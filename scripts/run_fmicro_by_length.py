...
logging.basicConfig(level=logging.INFO, format="%(asctime)s [fmicro-len] %(levelname)s %(message)s",
                    datefmt="%H:%M:%S")
...
def main():
    ...
    dense = base.dense_mean(acc_chunks, acc); n = len(acc)
    nch = np.array([acc_chunks[a].shape[0] for a in acc])
    log.info("n=%d  chunk-count dist: 1=%d  2=%d  3+=%d", n, int((nch == 1).sum()),
             int((nch == 2).sum()), int((nch >= 3).sum()))
    ...
    bins = {"all": np.ones(n, bool), "1-chunk": nch == 1, "2-chunk": nch == 2, "3+chunk": nch >= 3}
    arms = {"dense": dense}
    for k in args.k:
    ...
    d = rows_out["dense"]; s = rows_out.get("sparse_real_k128", {})
    for b in bins:
        if b in s:
            log.info("  %-9s  dense=%.4f  sparse_k128=%.4f  delta=%+.4f", b, d[b], s[b], s[b] - d[b])
    try:
        import mlflow
        ...
        with mlflow.start_run(run_name="dense-vs-sparse-by-size"):
            for name, vals in rows_out.items():
                for b, v in vals.items():
                    if v == v:  # not nan
                        mlflow.log_metric(f"{name}__{b}".replace("+", "p").replace("-", "_"), v)
