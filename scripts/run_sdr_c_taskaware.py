...
from protea_reranker_lab.sdr import (  # noqa: E402
    ...
    cosine_dense, tanimoto_dense,
)

logging.basicConfig(level=logging.INFO, format="%(asctime)s [taskaware] %(levelname)s %(message)s",
                    datefmt="%H:%M:%S")
...
def knn_fmicro(Bin, closures, knn=30):
    ...
    rn = np.linalg.norm(R, axis=1, keepdims=True); rn[rn == 0] = 1
    R = R / rn
    S = R @ R.T
...
def main():
    ...
    ti = np.array([p[0] for p in tp]); tj = np.array([p[1] for p in tp])
    ...
    c_hi = np.quantile(dcos, 0.90); l_lo = np.quantile(lin, 0.25)
    hardneg = (dcos > c_hi) & (lin < l_lo)
    w_task = 1.0 + args.pos_boost * (lin / max(lin.max(), 1e-6)) + args.neg_boost * hardneg.astype(np.float32)
    log.info("hard negatives: %.1f%% of train pairs | weight range %.2f-%.2f",
             100 * hardneg.mean(), float(w_task.min()), float(w_task.max()))
    Y = torch.tensor(lin); TI = torch.tensor(ti); TJ = torch.tensor(tj)
    ...
    GO = {"resnik": np.asarray(resnik_pairwise(clo_ev, ic_ev, ep), dtype=np.float64),
          "lin": np.asarray(lin_pairwise(clo_ev, ic_ev, ep, bic_ev), dtype=np.float64)}
    De = base.dense_mean(acc_chunks, [acc[i] for i in ev])
    dense_rho = {m: float(spearmanr(cosine_dense(De)[ii, jj], g)[0]) for m, g in GO.items()}
    _dense_fmicro = knn_fmicro((cosine_dense(De) > np.quantile(cosine_dense(De), 0.5)).astype(np.uint8),
                              clo_ev) if False else None  # dense uses real cosine below
    # dense f_micro via real cosine kNN
    def dense_knn_fmicro():
        Rn = l2n(De); S = Rn @ Rn.T; np.fill_diagonal(S, -1)
        return _fmicro_from_S(S, clo_ev)
    log.info("baselines: dense Spearman resnik=%.4f lin=%.4f", dense_rho["resnik"], dense_rho["lin"])

    def train(obj):
        enc = nn.Linear(d, args.dict); opt = torch.optim.Adam(enc.parameters(), lr=args.lr)
        bs = 16384; ALPHA = 20.0; w = W[obj]; sw = float(w.sum())
        for e in range(args.epochs):
            ...
            for b in range(0, len(tp), bs):
                ...
                inter = (si * sj).sum(1); sim = inter / (si.sum(1) + sj.sum(1) - inter + 1e-6)
    ...
    for obj in ("geometry", "taskaware"):
        ...
        rho = {m: float(spearmanr(tanimoto_dense(Bin, args.k)[ii, jj], GO[m])[0]) for m in GO}
        fm = knn_fmicro(Bin, clo_ev)
        out[obj] = {"spearman_resnik": rho["resnik"], "spearman_lin": rho["lin"], "fmicro": fm}
        log.info("  %s -> Spearman resnik=%.4f lin=%.4f | k-NN f_micro=%.4f",
                 obj, rho["resnik"], rho["lin"], fm)
        with mlflow.start_run(run_name=f"obj-{obj}"):
            mlflow.log_params({"objective": obj, "k": args.k, "dict": args.dict, "epochs": args.epochs,
                               "pos_boost": args.pos_boost, "neg_boost": args.neg_boost})
    ...
    log.info("  geometry : Spearman_r=%.4f f_micro=%.4f", g["spearman_resnik"], g["fmicro"])
    log.info("  taskaware: Spearman_r=%.4f f_micro=%.4f", t["spearman_resnik"], t["fmicro"])
    log.info("  >> task-aware vs geometry: dSpearman=%+.4f  dF_MICRO=%+.4f",
             t["spearman_resnik"] - g["spearman_resnik"], t["fmicro"] - g["fmicro"])
    return 0
