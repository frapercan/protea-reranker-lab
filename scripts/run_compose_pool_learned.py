...
def train_learned(Xn, tr, ev, clo_tr, clo_ev, dag, args, rng, label):
    ...
    Xtr = torch.tensor(Xn[tr])
    Xev = torch.tensor(Xn[ev])
    d = Xn.shape[1]
    ...
    for e in range(args.epochs):
        ...
        if e % EVAL_EVERY == 0 or e == args.epochs - 1:
            ...
            log.info(
                "  [%s] epoch %3d loss=%.4f | learned f_micro=%.4f",
                label,
                e,
                float(loss),
                f_sp,
...
def main():
    ...
    for pool, Xn in inputs.items():
        ...
        log.info(
            "=== input=%s : dense(no-learn) f_micro=%.4f -> training encoder ===",
            pool,
            f_dense,
        )
    ...
    bestcell = max(
        ((p, m) for p in inputs for m in ("dense", "learned")),
        key=lambda pm: table[pm[0]][pm[1]],
    )
    log.info(
        ">> BEST CELL: %s + %s = %.4f (%+.4f vs platform). Compose? maxabs-learned vs maxabs-dense = %+.4f, vs mean-learned = %+.4f",
        bestcell[0],
        bestcell[1],
        table[bestcell[0]][bestcell[1]],
        table[bestcell[0]][bestcell[1]] - base,
