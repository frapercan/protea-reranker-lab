...
import argparse, json, logging, subprocess, sys, tempfile
...
logging.basicConfig(level=logging.INFO, format="%(asctime)s [bench-enc] %(levelname)s %(message)s",
                    datefmt="%H:%M:%S")
    ...
    for cat in ("NK", "LK", "PK"):
        for ln in open(GT_DIR / f"groundtruth_{cat}.tsv"):
            ...
            if acc == "EntryID":
                continue
            key = (cat, asp)
            d = cells.setdefault(key, {"proteins": set(), "pairs": set(), "aspect": asp})
            d["proteins"].add(acc); d["pairs"].add((acc, go))
    return cells
...
def knn_transfer(Q, R, ref_closures, ref_terms_ix, knn, batch=1000):
    ...
    Rn = rl.l2n(R); Qn = rl.l2n(Q); nq = Qn.shape[0]; nt = len(ref_terms_ix)
    refT_r, refT_c = [], []
    for i, cl in enumerate(ref_closures):
        for t in cl:
            refT_r.append(i); refT_c.append(ref_terms_ix[t])
    refT = sp.csr_matrix((np.ones(len(refT_r), np.float32), (refT_r, refT_c)),
                         shape=(R.shape[0], nt))
    ...
    for b in range(0, nq, batch):
        ...
        bq = S.shape[0]
        w = np.take_along_axis(S, nbr, axis=1); w[w < 0] = 0
        rr = np.repeat(np.arange(bq), knn); cc = nbr.ravel()
        Sk = sp.csr_matrix((w.ravel(), (rr, cc)), shape=(bq, R.shape[0]), dtype=np.float32)
        out[sl] = (Sk @ refT)
    return out.tocsr()
...
def write_pred_gt(scores, query_ix, terms, cell, prots, tmpdir, max_terms=500):
    ...
    pred_dir = Path(tmpdir) / f"pred_{cell[0]}_{cell[1]}"; pred_dir.mkdir(parents=True, exist_ok=True)
...
def run_cafaeval(pred_dir, gt_path, tmpdir):
    drv = Path(tmpdir) / "drv.py"; drv.write_text(CAFAEVAL_DRIVER)
    r = subprocess.run([PROTEA_PY, str(drv), OBO, str(pred_dir), str(gt_path), IA],
                       capture_output=True, text=True)
    if r.returncode != 0:
        log.warning("cafaeval failed: %s", r.stderr[-800:]); return None
    return json.loads(r.stdout.strip().splitlines()[-1])


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--ref-n", type=int, default=80000, help="reference pool size (v227 t0 sample)")
    ...
    log.info("7401 GT: %d query proteins, cells: %s", len(queries),
    ...
    qset = set(queries)
    cur.execute("""SELECT DISTINCT protein_accession FROM protein_go_annotation
    ...
    rng.shuffle(ref_accs); ref_accs = ref_accs[:args.ref_n]
    ...
    cur.execute("""SELECT pga.protein_accession, gt.go_id FROM protein_go_annotation pga
    ...
    log.info("reference=%d (with closures) | queries embedded=%d/%d", len(ref_accs), len(q_accs), len(queries))

    terms = sorted({t for c in ref_clo for t in c}); tix = {t: i for i, t in enumerate(terms)}
    ...
    Y = torch.tensor(np.asarray(lin_pairwise(ref_clo, ic, tp, bic), dtype=np.float32), device=dev)
    TI = torch.tensor([p[0] for p in tp], device=dev); TJ = torch.tensor([p[1] for p in tp], device=dev)
    enc = nn.Linear(d, args.dict).to(dev); opt = torch.optim.Adam(enc.parameters(), lr=1e-3)
    bs = 32768; npr = len(tp)
    for e in range(args.epochs):
        Z = enc(Rt); opt.zero_grad(); loss = torch.zeros((), device=dev)
        for b in range(0, npr, bs):
            sl = slice(b, b + bs); zi, zj = Z[TI[sl]], Z[TJ[sl]]
    ...
    with torch.no_grad():
        ...
        Qc = rl.topk_real(enc(torch.tensor(rl.l2n(Q), device=dev)).cpu().numpy().astype(np.float32), args.eval_k)
    ...
    with tempfile.TemporaryDirectory() as td:
        for arm, (Qx, Rx) in arms.items():
            ...
            for cell, dmeta in sorted(cells.items()):
                pred_dir, gt_path = write_pred_gt(scores, query_ix, terms, cell, dmeta["proteins"], td)
                ...
                results[(arm, cell)] = fw
                log.info("  %-7s %s-%s  f_micro_w=%s", arm, cell[0], cell[1],
    ...
    for arm in ("dense", "learned"):
        nklk = [results[(arm, c)] for c in cells if c[0] in ("NK", "LK") and results.get((arm, c)) is not None]
        pk = [results[(arm, c)] for c in cells if c[0] == "PK" and results.get((arm, c)) is not None]
