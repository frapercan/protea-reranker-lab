...
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s [attnpool] %(levelname)s %(message)s",
    datefmt="%H:%M:%S",
)
...
def main():
    ...
    table["mean learned"], _ = train_loop(
        fwd_mean, list(enc.parameters()), tr, ev, clo_tr, clo_ev, dag, args, dev, "mean"
    )
    ...
    if _ml:
        mlflow.log_metric("mean_dense", mean_dense)
        mlflow.log_metric("mean_learned", table["mean learned"])
    ...
    for k in (
        "mean dense",
        "mean learned",
        "attn heads=1 learned",
        "attn heads=4 learned",
    ):
        if k in table:
            log.info(
                "  %-26s %.4f  (%+.4f vs mean-dense)", k, table[k], table[k] - base
            )
    best = max(table, key=lambda k: table[k])
    ml = table["mean learned"]
    log.info(
        ">> BEST: %s = %.4f | vs mean-learned (%.4f): %+.4f",
        best,
        table[best],
        ml,
        table[best] - ml,
