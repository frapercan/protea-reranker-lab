...
logging.basicConfig(level=logging.INFO, format="%(asctime)s [softpool] %(levelname)s %(message)s",
                    datefmt="%H:%M:%S")
...
class SoftPoolEnc(nn.Module):
    ...
    def __init__(self, d, dict_):
        super().__init__()
        self.raw_beta = nn.Parameter(torch.tensor(0.0))   # softplus(0)=0.69 -> mild peaking at init
        self.lin = nn.Linear(d, dict_)
...
def train_loop(forward_Z, params, tr, ev, clo_tr, clo_ev, dag, args, dev, label):
    ...
    Y = torch.tensor(np.asarray(lin_pairwise(clo_tr, ic_tr, tp, bic_tr), dtype=np.float32), device=dev)
    TI = torch.tensor([p[0] for p in tp], device=dev); TJ = torch.tensor([p[1] for p in tp], device=dev)
    opt = torch.optim.Adam(params, lr=args.lr)
    np_ = len(tp); bs = 32768; EVAL_EVERY = 15; PATIENCE = 6
    ...
    for e in range(args.epochs):
        ...
        if e % EVAL_EVERY == 0 or e == args.epochs - 1:
            ...
            extra = ""
            log.info("  [%s] epoch %3d loss=%.4f | f_micro=%.4f%s", label, e, float(loss), f_sp, extra)
            if _ml_on:  # live per-epoch curve, one metric series per arm/label
                key = label.replace("(", "_").replace(")", "").replace("+", "_p_").replace(" ", "")
...
def main():
    ...
    for nm, X in fixed.items():
        ...
        Xtr_idx = torch.tensor(tr, device=dev); Xev_idx = torch.tensor(ev, device=dev)
        ...
        bf, _ = train_loop(fwd, list(enc.parameters()), tr, ev, clo_tr, clo_ev, dag, args, dev, nm)
        table[f"{nm} learned"] = bf
        if _ml:
            mlflow.log_metric(f"learned_{nm.replace('(', '_').replace(')', '').replace('+', '_p_')}", bf)
    ...
    log.info("=== Exp2: learnable SOFT-POOL (softmax-magnitude temperature beta) + encoder ===")
    bf_sp, _ = train_loop(fwd_sp, list(sp.parameters()), tr, ev, clo_tr, clo_ev, dag, args, dev, "softpool")
    ...
    if _ml:
        mlflow.log_metric("softpool_learned", bf_sp); mlflow.log_metric("softpool_beta", learned_beta)

    log.info("=== SUMMARY (held-out f_micro, same split) ===")
    order = ["mean dense", "maxabs dense", "mean learned", "maxabs learned",
             "concat(mean+maxabs) learned", "softpool learned"]
    base = table.get("mean dense", 0.0)
    for k in order:
        if k in table:
            log.info("  %-28s %.4f  (%+.4f vs mean-dense)", k, table[k], table[k] - base)
    best = max(table, key=lambda k: table[k])
    log.info(">> BEST: %s = %.4f (%+.4f vs mean-dense) | learned soft-pool beta=%.4f",
             best, table[best], table[best] - base, learned_beta)
    if _ml:
        mlflow.end_run()
    return 0
