"""Board-faithful BP scoring for the Phase 1 (GCN joint-scorer) A/B, mirroring Phase 0.

cell = naivemax( reranker_BP_test  UNION  InterPro2GO_BP_test ), scored with cafa_eval
(OBO, IA, TOI, prop=fill, norm=cafa, no_orphans, PK excludes PK_known). Reports best
f_micro_w (+tau, pr, rc, cov), realized recall at best-Fmax tau, and propagated recall
ceiling. Reuses the literature_infame graft harness.
"""
...
def score_bp_full(pred_rows, cat):
    ...
    d = tempfile.mkdtemp(prefix=f"bp1_{cat}_")
    ...
    btau = float(bbp["tau"])
    rc_ceiling = float(bp["rc_micro_w"].max())
...
def main():
    ...
    for cat in ["lk", "pk"]:
        champ = G.champion_bp(cat)
        base = grafted(cat, "base")
        union = grafted(cat, "union")
        out["cells"][cat] = {
            "champion_reproduced": champ, "retrained_base": base, "union_pool": union,
        ...
        print(cat, "champ", champ["f_micro_w"], "base", base["f_micro_w"], "union", union["f_micro_w"],
              "| rc_ceiling base->union", base["rc_ceiling_tau0"], "->", union["rc_ceiling_tau0"], flush=True)
