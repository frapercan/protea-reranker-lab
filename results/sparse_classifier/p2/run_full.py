"""FULL: 7-seed ensemble two-tower sparse functional candidate generator.
Trains on the full train split (90% of 88212), full vocab, saves per-seed ckpts, evaluates
top-k recall on the held-out 10% protein split vs the term-frequency baseline."""
...
print(f"vocab V={data.V} train={len(data.train_idx)} test={len(data.test_idx)} dev={DEV}")

models, all_losses = [], {}
for sd in SEEDS:
    m, losses = train_seed(data, sd, data.train_idx, CFG, log_prefix="FULL ")
    ...
    print(f"  seed{sd} solo recall@100={rec:.4f}  GPUmem={torch.cuda.max_memory_allocated()/1e9:.2f}GB")
    all_losses[sd] = losses
    models.append(m)

# ensemble + baselines + recall at several k
tf100 = recall_tf_baseline(data, data.test_idx, 100)
metrics = {"vocab_size": int(data.V), "n_train": int(len(data.train_idx)),
           "n_test": int(len(data.test_idx)), "seeds": SEEDS, "cfg": CFG,
           "losses_per_seed": all_losses, "tf_baseline_recall100": tf100,
           "recall_at_k": {}, "solo_recall100": {}}
for k in [50, 100, 200]:
    ...
    metrics["recall_at_k"][str(k)] = {"ensemble": rec, "ceiling": ceil,
                                      "tf_baseline": tfk, "n": n}
    print(f"k={k}: ensemble={rec:.4f}  tf={tfk:.4f}  ceiling={ceil:.4f}")

# stratify recall@100 by number of true propagated positives
@torch.no_grad()
def stratified(models, eval_idx, k=100, bs=512):
    ...
    return {k2: {"mean_recall": float(np.mean(v)), "n": len(v)} for k2, v in buckets.items() if v}
...
print(f"\nDONE in {time.time()-t0:.0f}s. metrics -> {OUT}/metrics.json")
