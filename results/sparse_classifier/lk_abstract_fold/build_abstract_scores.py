"""ABSTRACT-augmented text->GO max-cosine scores for the LK-BP (protein,go) pairs
(train v160..v225 + test v227-v230). Mirrors literature_infame/build_text_scores
_infame.py EXACTLY, but the protein sentence set adds pre-cut PubMed ABSTRACT
bodies on top of desc + FUNCTION comments + pre-cut reference TITLES.

Single temporal cut: publications <= 2025-09 (v227 t0). Encoder = S-PubMedBERT-
MS-MARCO. Output: abstract_text_scores.parquet (acc, go, cos, text_score[0,1]).
Cut-invariant -> one score per unique (protein,go), joined to all rows downstream.
"""
import os
import re
import json
...
CACHE = os.path.join(HERE, "..", "literature_infame", "uniprot_cache.json")
ABSTR = os.path.join(HERE, "abstracts.json")
PAIRS = os.path.join(HERE, "lk_bp_pairs.parquet")
...
def protein_sentences(rec, abstracts):
    """desc + FUNCTION comments + pre-cut reference TITLES + pre-cut ABSTRACT bodies."""
    sents = []
    if rec.get("desc"):
    ...
    for s in sents:
        ...
        if k not in seen:
            seen.add(k)
            out.append(s)
    return out[:128]  # wider cap than titles (64) to admit abstract bodies
...
def main():
    ...
    print("building protein sentences (title+abstract)...", flush=True)
    prot_sents = {}
    for p in proteins:
        rec = cache.get(p)
        prot_sents[p] = protein_sentences(rec, abstracts) if rec else []
    ...
    sent_emb_t = torch.from_numpy(sent_emb).to(DEV)
    ...
    for p, grp in cand_by_prot:
        ...
        if not sents:
            ...
            S = sent_emb_t[[sidx[s] for s in sents]]
    ...
    fn = os.path.join(HERE, "abstract_text_scores.parquet")
    out.to_parquet(fn)
    cov = {"pairs_scored": len(out),
           ...
           "proteins_with_abstract": int(sum(
               1 for p in proteins if cache.get(p) and any(
                   r.get("pmid") and is_precut(r) and str(r["pmid"]) in abstracts
                   for r in cache[p].get("refs", [])))),
           "cos_min": round(lo, 4), "cos_med": round(float(out.cos.median()), 4),
           "cos_max": round(hi, 4)}
    json.dump(cov, open(os.path.join(HERE, "abstract_text_coverage.json"), "w"),
              indent=2)
