...
accession batches. Writes protein2ipr.json {acc: [IPRxxxxxx,...]}.
"""
import json, os, sys, time
import requests

HERE = os.path.dirname(os.path.abspath(__file__))
accs = [l.strip() for l in open(os.path.join(HERE, "accessions.txt")) if l.strip()]
...
json.dump(out, open(os.path.join(HERE, "protein2ipr.json"), "w"))
