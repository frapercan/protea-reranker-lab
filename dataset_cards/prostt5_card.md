...
tags:
  ...
  - lightgbm
  - benchmark
pretty_name: "bench-v1 v226-lineage prostt5 (K=3,5,10)"
zenodo_doi: TBD
---

# bench-v1-K{3,5,10}-v226-lineage-prostt5

Dataset family produced by PROTEA's `export_research_dataset` operation
for the ProstT5 protein language model. Each cell in the K axis is a
...
| --- | --- |
| PLM key | `prostt5` |
| HuggingFace checkpoint | `Rostlab/ProstT5` |
| Architecture | ProstT5 (T5 backbone, AA + 3Di joint vocabulary; AA-only inference) |
| Embedding dimension (raw) | 1024 |
| Pooling strategy | Mean pool over residue axis |
| PCA applied | PCA(16) per config, fit on full pool (transductive, unsupervised) |
| Embedding config id | `c0ae5b69-d6dc-41cf-a711-1739d3d2e170` |

Note: the K5 cell uses the same embedding config id as the legacy
`bench-v1-K5-v226-lineage` dataset (which was produced before the
PLM-explicit naming scheme, ADR D36). That legacy dataset and the
canonical `bench-v1-K5-v226-lineage-prostt5` are produced from the
same embedding pool.
...
| --- | --- | --- | --- | --- |
| bench-v1-K3-v226-lineage-prostt5 | TBD | TBD | 6d97a624b8a7 | Pending EXP.13 completion |
| bench-v1-K5-v226-lineage-prostt5 | 24,351,779 | 1,066,859 | 6d97a624b8a7 | Available |
| bench-v1-K10-v226-lineage-prostt5 | TBD | TBD | 6d97a624b8a7 | Pending EXP.13 completion |

Note: K3 and K10 row counts will be updated once EXP.13 finishes.
...
**K3 and K10 cells pending.** Row counts for K=3 and K=10 are
placeholders. Final numbers arrive when EXP.13 finishes (ETA
2026-05-25).

**Legacy alias.** The `bench-v1-K5-v226-lineage` dataset (no PLM
suffix) uses the same embedding config (`c0ae5b69`) and is the
pre-D36 predecessor of this cell.
