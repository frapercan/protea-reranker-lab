...
tags:
  ...
  - lightgbm
  - benchmark
pretty_name: "bench-v1 v226-lineage prot_t5 (K=3,5,10)"
zenodo_doi: TBD
---

# bench-v1-K{3,5,10}-v226-lineage-prot_t5

Dataset family produced by PROTEA's `export_research_dataset` operation
for the ProtT5-XL (half precision) protein language model. Each cell in
the K axis is a frozen LightGBM feature dataset covering one (K,
split-window) combination. These are the primary research deliverables
for the thesis per the 2026-05-25 decision (memory
`dl-postponed-2026-05-25`).
...
| --- | --- |
| PLM key | `prot_t5` |
| HuggingFace checkpoint | `Rostlab/prot_t5_xl_half_uniref50-enc` |
| Architecture | ProtT5-XL (T5 encoder, half precision) |
| Embedding dimension (raw) | 1024 |
| Pooling strategy | Mean pool via `prott5_encoder.embed_sequences` (vendored encoder) |
...
Note: the canonical config is the half-precision variant
(`084943c6`). The full-precision config (`db4db5ed`) is
classified as superseded (ADR D35).
...
| --- | --- | --- | --- | --- |
| bench-v1-K3-v226-lineage-prot_t5 | TBD | TBD | 6d97a624b8a7 | Pending EXP.13 completion |
| bench-v1-K5-v226-lineage-prot_t5 | TBD | TBD | 6d97a624b8a7 | Pending EXP.13 completion |
| bench-v1-K10-v226-lineage-prot_t5 | TBD | TBD | 6d97a624b8a7 | Pending EXP.13 completion |

Row counts will be updated once EXP.13 finishes. Based on comparable
PLMs (527 855 proteins in the pool), expected K5 train rows are in the
...
527,855 proteins with embeddings stored under
`embedding_config_id = 084943c6-fec1-441d-bdc5-63b0268ada1b` (fully
hydrated 2026-05-20, see ADR D35 and PROTEA PR #418).
...
**All K cells pending.** This PLM was the last to complete hydration
(2026-05-20); EXP.13 export jobs are in-flight as of 2026-05-25.
