...
def main():
    ...
    for cat in ("NK", "LK", "PK"):
        ...
        b.save_model(f"{OUT}/ensemble_gbm_{cat}.txt")
        pred = b.predict(Xte)
        ...
        res[cat] = v
        imp = {n: int(x) for n, x in zip(FEATURES, b.feature_importance("gain"))}
    ...
    print(f"saved boosters + feature_spec -> {OUT}", flush=True)
