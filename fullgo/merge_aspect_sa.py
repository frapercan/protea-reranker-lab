...
merged = os.path.join(W, "pred_aspectaware_seedavg.tsv")
with open(merged, "w") as w:
    for line in open(os.path.join(W, "pred_gotext_seedavg.tsv")):
        if line.split("\t")[1] in BP: w.write(line)
    for line in open(os.path.join(W, "pred_anc2vec_seedavg.tsv")):
...
aw = nine(merged)
anc = json.load(open(os.path.join(W, "eval_9cell_seedavg.json")))["anc2vec"]
...
json.dump({"anc2vec": anc, "aspect_aware": aw}, open(os.path.join(W, "eval_aspectaware_seedavg.json"), "w"), indent=2)
