...
def load_se(cell):
    Z = np.load(KW / f"sse_scores_{cell}.npz", allow_pickle=True)
...
results = {"combine": COMBINE, "model": {
    "name": "SSE (Sparse Semantic-Entailment) POC over dense ESM2-3B, from scratch, on a subsample",
    "architecture": "everything a sparse SDR (N=2048). protein tower: ESM2-3B mean(2560) -> MLP -> "
                    "soft k-WTA(128); term tower: embedding -> soft k-WTA(per-term m in [16,112] by "
                    "ontology depth); entailment score = coverage |supp(x_p) cap supp(z_t)|/|supp(z_t)|; "
                    "5 independently-seeded code sets, consensus S = MIN_k s_k.",
    "axioms": "t0 2025-07-22 GO EL normal forms as differentiable soft set-containment penalties: "
              "nf1 subsumption supp(z_D) subset supp(z_C); nf4 existential supp(R_r.z_D) subset "
              "supp(z_C) with a learned sparse relation mask R_r (child covers parent's features).",
    "temporal_gate": "labels = generator_frames t0/v227 propagated BP; eval LK/PK proteins HELD OUT of "
                     "the training subsample (no per-protein memorization); eval-window-new BP terms "
                     "blind by construction. LK has no -known exclusion -> any LK positive is suspect.",
    "corpus": "25,000-protein subsample of the 88k experimental set; 5 seeds x 12 epochs"}}
