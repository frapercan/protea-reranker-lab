...
def score(cat, tau_pre, rescale=False):
    """One board-shaped evaluation: our submission for `cat`, against that cell's own gt.

    cafaeval sweeps tau over [0,1]; lambdarank margins run to 10.9, so a raw-margin submission
    puts a large share of its own positives permanently above every threshold. Measured share
    above 1.0: pk 3.40%, nk 34.32%, lk 61.75% -- and the deviation from the board's published
    figure tracks it monotonically (-0.005 / -0.035 / -0.136). `rescale` maps the SURVIVORS of
    the prefilter onto (0.01, 1.0]. It is monotone, so it cannot reorder anything, and because
    the prefilter has already fixed which cells are zero it cannot change what `fill` inherits
    either -- which is exactly what made the old global rankpct map cost 0.088.
    """
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        d.mkdir()
        vals = RR[keep]
        if rescale and len(vals):
            lo, hi = vals.min(), vals.max()
            vals = 0.01 + 0.99 * (vals - lo) / (hi - lo) if hi > lo else np.full(len(vals), 1.0)
        with (d / "predictions_protea.tsv").open("w") as fh:
            for p_, g_, v in zip(PROT[keep], GO[keep], vals):
...
ARMS = ((0.0, False, "guard_only"), (0.4, False, "prefilter_0.4"),
        (0.0, True, "guard_only_rescaled"), (0.4, True, "prefilter_0.4_rescaled"))
for tau_pre, rs, label in ARMS:
    res["arms"][label] = {}
    for cat in ("NK", "LK", "PK"):
        r = score(cat, tau_pre, rs)
        ...
        json.dump(res, open(OUT / "build_submission_rescale.json", "w"), indent=1)
...
for cat in ("NK", "LK", "PK"):
    ...
    p = res["arms"]["prefilter_0.4"][cat].get("biological_process", {}).get("f_micro_w")
    gr = res["arms"]["guard_only_rescaled"][cat].get("biological_process", {}).get("f_micro_w")
    pr_ = res["arms"]["prefilter_0.4_rescaled"][cat].get("biological_process", {}).get("f_micro_w")
    b = BOARD[cat]
    print(f"  {cat}-BP  guard {g}  prefilter@0.4 {p}  guard+rescale {gr}  "
          f"prefilter+rescale {pr_}   board published {b}", flush=True)
    if g is not None:
