...
IAS = {
    "A_lafa_t0_Sep_2025": "/home/frapercan/Thesis2/protea-lafa-knn/lafa_t0_Sep_2025/IA.tsv",
    "B_lab_recompute_v227": "/home/frapercan/Thesis2/repositories/protea-reranker-lab/datasets/ia/IA-swissprot-exp-v227.txt",
    "C_generic_cafa6_REJECTED": "/home/frapercan/Thesis2/repositories/PROTEA/data/benchmarks/IA_cafa6.tsv",
}
...
t0 = time.time()
res = {"question": "which IA table does the board score us with?",
...
for name, ia in IAS.items():
    if not Path(ia).exists():
        print(f"  {name}: MISSING {ia}", flush=True)
        continue
    with tempfile.TemporaryDirectory() as td:
        ...
        res["arms"][name] = {"ia_file": ia, "f_micro_w": f,
    ...
    json.dump(res, open(Path(__file__).parent / "which_ia_is_the_boards.json", "w"), indent=1)
