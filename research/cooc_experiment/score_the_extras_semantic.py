...
def score_and_cafa(extras_by_prot):
    """Score a {prot: [terms]} extra set with S over feats(), submit the better half, run cafaeval."""
    ex_p, ex_g = [], []
    for prot, gs in extras_by_prot.items():
        for g in gs:
    ...
    sc = S_.predict(Xex, num_iteration=S_.best_iteration)
    thr = np.quantile(sc, 0.5)                    # submit the better half; the fill tax is real
    rows = base + [(p_, g_, float(v)) for p_, g_, v in zip(ex_p, ex_g, sc) if v > thr]
    return cafa(rows), len(ex_p)

for TOPK in (100, 200, 400):
    cls = classifier_extras(TOPK)
    sem = semantic_extras(TOPK)
    uni = {p: sorted(set(cls.get(p, [])) | set(sem.get(p, []))) for p in eprots}
    arms = {}
    for tag, ex in (("B_classifier", cls), ("B_semantic", sem), ("B_union", uni)):
        f_, n_ = score_and_cafa(ex)
        arms[tag] = f_
        arms[f"{tag}_delta"] = round(f_ - fa, 5) if f_ is not None else None
        arms[f"{tag}_n"] = n_
    res["arms"][f"top{TOPK}"] = arms
    print(f"  top{TOPK:<3}: classifier {arms['B_classifier']} ({arms['B_classifier_delta']:+.5f}, "
          f"n={arms['B_classifier_n']:,}) | semantic {arms['B_semantic']} "
          f"({arms['B_semantic_delta']:+.5f}, n={arms['B_semantic_n']:,}) | union {arms['B_union']} "
          f"({arms['B_union_delta']:+.5f}, n={arms['B_union_n']:,})   ({time.time()-t0:.0f}s)", flush=True)
    json.dump(res, open(W / "score_the_extras_semantic.json", "w"), indent=1)

def best(tag):
    return max((v[f"{tag}_delta"] for v in res["arms"].values() if v.get(f"{tag}_delta") is not None),
               default=None)
bC, bS, bU = best("B_classifier"), best("B_semantic"), best("B_union")
res["verdict"] = {"best_classifier": bC, "best_semantic": bS, "best_union": bU,
                  "SEMANTIC_CLEARS_FLOOR": bool(bS is not None and bS > 0.0034),
                  "UNION_BEATS_CLASSIFIER": bool(bU is not None and bC is not None and bU > bC + 0.0034),
                  "COMPLEMENTARY": bool(bU is not None and bC is not None and bS is not None
                                        and bU > max(bC, bS) + 0.0034)}
json.dump(res, open(W / "score_the_extras_semantic.json", "w"), indent=1)
print(f"\n=== does the semantic generator pay? ({time.time()-t0:.0f}s) ===", flush=True)
print(f"  A pool only (deployed)          {fa}", flush=True)
print(f"  best B_classifier (reference)   {bC:+.5f}", flush=True)
print(f"  best B_semantic  (text channel) {bS:+.5f}", flush=True)
print(f"  best B_union     (both)         {bU:+.5f}", flush=True)
print(f"  semantic clears 0.0034 floor: {res['verdict']['SEMANTIC_CLEARS_FLOOR']} | "
      f"union beats classifier: {res['verdict']['UNION_BEATS_CLASSIFIER']} | "
      f"complementary: {res['verdict']['COMPLEMENTARY']}", flush=True)
