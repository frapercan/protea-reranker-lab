...
t0 = time.time()
# THE CONTROL. A dumb per-window term prior: how often does this term appear among the
# TRAIN proteins' BP answers? It ignores the protein's known set completely, so it
# carries NO cross-aspect structure whatsoever, only "what is being annotated in this
# window". If this alone reproduces the +0.0896, the set-level result was a temporal
# leak and the cross-aspect hypothesis is dead.
import collections as _c
tf = _c.Counter()
for _p in src:
    tf.update(BP[_p])
_n = max(len(src), 1)
new = np.array([tf.get(g, 0) / _n for g in go], dtype=np.float32)
print(f"window term prior built; nonzero on {(new > 0).mean():.1%} of rows "
...
def run(name, Xm, names):
    ...
    imp = None
    if "window_term_prior" in names:
        gains = b.feature_importance("gain")
        k = names.index("window_term_prior")
...
res = {"note": "CONTROL: the set-level column replaced by a DUMB per-window term frequency prior that ignores the known set entirely. If this reproduces set-level's +0.0896, that result was a temporal leak (knowing what this window annotates), not cross-aspect structure. Identical rows; the only "
...
json.dump(res, open(W / "window_prior_control.json", "w"), indent=1)

XB = np.hstack([X, new.reshape(-1, 1)])
b_, bimp, bi = run("B", XB, FEATS + ["window_term_prior"])
res["B_plus_window_prior"] = b_
res["B_best_iter"] = bi
res["window_prior_gain_share_pct"] = bimp
print(f"[B] + window_term_prior  {b_}   (feature gain share: {bimp}%)  ({time.time()-t0:.0f}s)", flush=True)
...
json.dump(res, open(W / "window_prior_control.json", "w"), indent=1)
print("\n=== CONTROL: is the gain just a window term prior? ===", flush=True)
...
if fa and fb:
    ...
    print("  If this ~matches set-level +0.0896 -> temporal leak, cross-aspect is DEAD.", flush=True)
    print("  If this is small -> set-level carries real structure beyond the window prior.", flush=True)
