...
SEED, VAL_PAIR, TAU = 42, "v225-v227", -1e9
...
with (REL / "groundtruth_LK.tsv").open() as fh:
    next(fh)
    for line in fh:
...
df,dfs=cafa_eval("{obo}","{pd}","{gt}",ia="{ia}",prop="fill",norm="cafa",no_orphans=True,
    toi_file="{toi}",max_terms=500,th_step=0.01,n_cpu=4,weighted_only=False)
out={{}}
...
res = {"anchor_prefiltered_arm": ANCHOR, "A_pool_only": fa,
       ...
       "PRECONDITION": bool(fa is not None and abs(fa - ANCHOR) < 0.006),
...
if not res["PRECONDITION"]:
    ...
    json.dump(res, open(W / "score_the_extras_lk.json", "w"), indent=1)
    raise SystemExit(0)
