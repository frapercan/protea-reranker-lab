...
RRmap = dict(zip(zip(np.asarray(t.column("protein_accession").to_pylist()),
                     np.asarray(t.column("go_term_id").to_pylist())),
                 t.column("reranker_score").to_numpy(zero_copy_only=False).astype(np.float64)))
...
RR = np.array([RRmap.get((p_, g_), np.nan) for p_, g_ in zip(P, G)])
pk = C == "pk"
...
res = {"question": "do the prefilter and the classifier arm compose, or are they one lever twice?",
       ...
       "precondition": f"arm A fold-mean must land within 0.02 of {ANCHOR_A} or the run is void.",
...
for i in range(K):
    ...
    json.dump(res, open(W / "compose_prefilter_classifier.json", "w"), indent=1)
...
anchor_ok = A_mean is not None and abs(A_mean - ANCHOR_A) < 0.02
