...
ARMS = [
    ("L_lab_as_run_all_night", None, 500, 0.001),
    ("T_plus_toi", TOI, 500, 0.001),
    ("M_plus_maxterms_default", TOI, None, 0.001),
    ("B_board_exact_line", TOI, None, 0.01),
]
t0 = time.time()
...
for name, toi, mt, ts in ARMS:
    ...
    json.dump(res, open(W / "board_exact_flags.json", "w"), indent=1)
