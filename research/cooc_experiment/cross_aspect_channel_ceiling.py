...
def knn_transfer(kq, topn=30):
    """Transfer BP terms from the train proteins whose KNOWN set resembles kq."""
...
t0 = time.time()
res = {"note": "predict NEW BP terms from t0-known terms ALONE. Same info to every arm; "
               "leakage-symmetric, so read S minus C, not the absolutes.",
...
json.dump(res, open(W / "cross_aspect_channel_ceiling.json", "w"), indent=1)
