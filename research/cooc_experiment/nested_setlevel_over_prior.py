...
src_ = [p for p in TRp if p in K and BP.get(p)]
src_sets = [K[p] for p in src_]
...
for _p in src_:
    tf.update(BP[_p])
_n = max(len(src_), 1)
prior = np.array([tf.get(g, 0) / _n for g in go], dtype=np.float32)

# A': 72 + the dumb window prior. This is the honest baseline: it already contains
# everything the temporal artefact can give.
XA = np.hstack([X, prior.reshape(-1, 1)])
NA = FEATS + ["window_term_prior"]
a, _, ai = run("Aprior", XA, NA)
res["A_72_plus_window_prior"] = a
print(f"[A'] 72 + window prior          {a}", flush=True)
json.dump(res, open(W / "nested_setlevel_over_prior.json", "w"), indent=1)

# B': + set_level ON TOP. Any gain here is what the SET structure adds that the
# window artefact cannot explain.
XB = np.hstack([XA, new.reshape(-1, 1)])
NB = NA + ["set_level_transfer"]
b_, bimp, bi = run("Bnested", XB, NB)
res["B_plus_set_level_over_prior"] = b_
res["set_level_gain_share_pct_nested"] = bimp
print(f"[B'] + set_level ON TOP        {b_}   (set_level gain share: {bimp}%)", flush=True)

fa = (a or {}).get("f_micro_w"); fb = (b_ or {}).get("f_micro_w")
if fa and fb: res["set_level_adds_over_prior"] = round(fb - fa, 4)
json.dump(res, open(W / "nested_setlevel_over_prior.json", "w"), indent=1)
print("\n=== Does cross-aspect SET structure add anything the window prior cannot? ===", flush=True)
print(f"  A' 72 + window prior            = {fa}", flush=True)
print(f"  B' + set_level_transfer         = {fb}", flush=True)
if fa and fb:
    print(f"  -> set-level adds {fb-fa:+.4f} over the artefact", flush=True)
    print("  ~0 => the cross-aspect hypothesis is DEAD: it was the window prior all along.", flush=True)
