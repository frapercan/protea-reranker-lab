...
SEED, K = 42, 10
ANCHOR = {"pk": 0.21269, "lk": 0.30376, "nk": 0.3021}    # build_submission.py, guard_only, BP

# --- the scored table (reranker) -------------------------------------------------
t = pq.read_table(W / "rerank_out" / "eval_scores.parquet")
key_s = list(zip(np.asarray(t.column("protein_accession").to_pylist()),
                 np.asarray(t.column("go_term_id").to_pylist())))
RR = dict(zip(key_s, t.column("reranker_score").to_numpy(zero_copy_only=False).astype(np.float64)))
CATS = dict(zip(key_s, np.asarray(t.column("category").to_pylist())))
ASPS = dict(zip(key_s, np.asarray(t.column("aspect").to_pylist())))
...
RRv = np.array([RR.get((p_, g_), np.nan) for p_, g_ in zip(P, G)])
print(f"feature rows {len(P):,} | reranker matched on {np.isfinite(RRv).mean():.1%}", flush=True)
...
for cell in ("pk", "lk", "nk"):
    ...
    for i in range(K):
        ...
        json.dump(res, open(W / "strategy_gate_crossfit.json", "w"), indent=1)
